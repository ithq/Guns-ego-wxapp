...
App({
  ...
  glbParam: {
    ...
    serveiceTelNo: "400-666-2808", //服务电话
    ...
    utl.wxRequset("wxapp", "deviceGH.htm", {
      ...
      if (res.data.result == 'error') {
        ...
        if (responseInfo != null && !isNaN(responseInfo)) {
          ...
          if (size > 0) {
            utl.confirmDialog("提示", "您名下有多个充电器，请点'确定'后，扫要归还的充电宝二维码继续归还！", function (rs) {
  ...
  saveFormId: function (formId, isPrePayId) {
    ...
    utl.wxRequset("wxapp", "saveFormId", param, function (success) {}, function (fail) {
  ...
  clickAdvertInfo: function (type, businessId) {
    ...
    if (advertInfo) {
      if (advertInfo.navigateTo == "url") {
        // 广告点击
        wx.setStorageSync("webViewUrl", advertInfo.url);
        wx.navigateTo({
   ...
   */
  showRewardedVideoAd(object, adUnitId) {
    let that = this;
