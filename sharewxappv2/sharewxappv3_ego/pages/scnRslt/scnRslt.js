...
Page({
  ...
  onLoad: function (options) {
    ...
    //6.处理广告图片
    var advertImage = '/img/img/bar1.png';
    ...
    app.showInterstitial('adunit-3d3b447d28a08d44');
    // 8.初始化完成
    this.setData({
      isInit: true
    })
  ...
  bleInit: function () {
    let that = this;
    // 从缓存读取连接状态
    let bleConnected = wx.getStorageSync('bleConnected');
    if (bleConnected) {
      wx.removeStorageSync('bleConnected');
      that.setData({
        bleConnected: true
      })
      return true;
  ...
  onShow: function (res) {
    var that = this;
    if (that.data.payOutTradeNo) { //从被打开的小程序返回
      // let queryId = app.glbParam.queryId;
      // app.glbParam.queryId = null;
      // console.info(queryId);
      // that.queryPayScoreOrderStatus(queryId);
    } else {
          ...
          wx.requestPayment({
            ...
            'success': function (success) {
              ...
              if (success.errMsg == 'requestPayment:ok') {
                ...
                //4. 重新处理 我要充电的业务逻辑。。。提高成功率，在这里延迟500ms
                setTimeout(() => {
                  console.log("充电。。。")
            ...
            fail: function (res) {
              ...
              app.endOperate(that);
              //取消重复点击结束=============
              //utls.altDialog('提示', res.err_desc || res.errMsg || "支付失败", '确定', null)
              // fail
              return 'error';
            },
  ...
  myWantToReCharge: function (e) {
    ...
    try {
      ...
      if (yfjAmt == 0) {
        app.endOperate(that);
        // 免费充电需要先看广告
        app.showRewardedVideoAd(that, 'adunit-876710fc149abc8d').then(res => {
          app.firstOperate(that, "租借中...");
          ...
          utls.altDialog('友情提示', '您使用的免费次数已使用完，请稍后再试', '知道了');
        })
      } else if (needFee > 0) {
        //余额不够，充值再充电
        var code = app.glbParam.code;
  ...
  onPlayVoice: function () {
    ...
    that.data.audioContext.src = "/img/voice/recharger.mp3";
  ...
  showAuthView: function () {
    wx.setStorageSync("webViewUrl", app.glbParam.rqstBaseUrl + 'appConfig/wxDebitAuthorization');
    wx.navigateTo({
      url: '../webView/webView'
    });
  },
  authCheckBoxChange: function (e) {
    this.setData({
      authCheckFlag: e.detail.value.length > 0 && e.detail.value[0] == 1
    })
  },
  // 免押租借
  payScoreToReCharge: function (e) {
    var that = this;
    if (!app.firstOperate(that, "租借中...")) {
      return;
    }
    console.info('免押租借中...')
    if (!that.data.authCheckFlag) {
      //取消重复点击结束=============
      app.endOperate(that);
      utls.altDialog('提示', '请先勾选同意协议。', '确定', null);
      return;
    }
    // 有免押订单提示免押租借
    ...
    try {
      ...
      if (null == payOutTradeNo || '' == payOutTradeNo) {
        ...
        var ssn_3rd = app.glbParam.ssn_3rd;
        //needFee:所需费用，feeTypeId：费用类型,custNo:客户编号
        that.payScoreRecharge(chargerId, needFee, feeTypeId, custNo, code, ssn_3rd);
      } else {
        //创建免押订单后，开始充电
        var code = app.glbParam.code;
        var ssn_3rd = app.glbParam.ssn_3rd;
  ...
  //免押租借方法
  payScoreRecharge: function (chargerId, needFee, feeTypeId, custNo, code, ssn_3rd) {
    console.log("免押租借")
    var that = this;
    var keycode = cmmn.getKeyCode([chargerId, needFee, feeTypeId, custNo], code)
    console.log(keycode)
    utls.wxRequset("wxapp", "getPrepayForPayScore", {
        session3rd: ssn_3rd,
        keyCode: keycode,
        chargerId: chargerId,
        needFee: needFee,
        ...
        try {
          ...
          if (wx.openBusinessView) {
            wx.openBusinessView({
              ...
              extraData: {
                mch_id: responseInfo.mchId,
                package: responseInfo.packageStr,
                timestamp: responseInfo.timestamp,
                nonce_str: responseInfo.nonceStr,
                sign_type: responseInfo.signType,
                sign: responseInfo.sign
              },
              success() {
                console.info('租借成功,订单号=' + responseInfo.outTradeNo)
                //保存全局订单号，重新调用我要充电
                that.setData({
                  payOutTradeNo: responseInfo.outTradeNo
                });
            ...
            app.endOperate(that);
            //引导用户升级微信版本
            utls.altDialog('提示', '您使用的微信版本过低，不支持免押支付，请升级版本。', '确定', null);
            return 'error';
          }
        } catch (eee) {
          //取消重复点击结束=============
   ...
   * 免押订单，充电获取密码方法
   */
  getPasswordForPayScore: function (custNo, feeTypeId, chargerId,
    payOutTradeNo, needFee, chargeRslt, latX, lonY, addr, code, ssn_3rd) {
    //1. 请求后台，生成我要充电的订单
    var that = this;
    //1.1 参数加密
    var keycode = cmmn.getKeyCode([custNo, feeTypeId, chargerId, payOutTradeNo, needFee, chargeRslt, latX, lonY, addr], code);
    //2. 请求后台处理充电.
    utls.wxRequsetForPost("wxapp", "getPasswordForPayScore", {
      custNo: custNo,
      feeTypeId: feeTypeId,
      chargerId: chargerId,
      outTradeNo: payOutTradeNo,
   ...
   * 查询订单状态函数
   * 由商家后台服务提供
   * @param query_id {string} 单据id，可以在接口【查询订单】进行单据查询
   */
  queryPayScoreOrderStatus: function (queryId) {
    ...
    var code = app.glbParam.code;
    var ssn_3rd = app.glbParam.ssn_3rd;
    var keycode = cmmn.getKeyCode([custNo, queryId], code);
    //2. 请求后台处理充电.
    utls.wxRequsetForPost("wxapp", "getTradeInfoByPayScore", {
      custNo: custNo,
      queryId: queryId,
      keyCode: keycode,
      session3rd: ssn_3rd
      ...
      //2. 调用免押支付功能 我要免押充电的业务逻辑。。。
      if (success.data.responseInfo.state == "DOING" && success.data.responseInfo.stateDescription == "USER_CONFIRM") {
        console.log("免押充电。。。")
        that.payScoreToReCharge();
      }
