...
Page({
  ...
  data: {
    ...
    custNo: '',
    chargerId: '',
    messageShowView: true, //展示蓝牙开启蓝牙model true true true 
    needBluetooth: false, //是否必须开启蓝牙
    bleOpenFlag: false, //是否已开启蓝牙
    bleName: '', //蓝牙名称
    bleCode: '', //蓝牙激活数据
    bleCodeResult: '', //蓝牙回复数据
    bleCharging: false, //蓝牙充电结果 false false false
    ...
    showTitle: '请打开手机蓝牙和GPS位置定位，靠近充电器激活充电',
  ...
  bleOpenBluetoothEvent: function (res) {
    ...
    if (res) {
      ...
      if (that.data.needBluetooth) {
        util.alertDialog('提示', '请手工开启蓝牙，才能进行充电', '确定', null)
      } else {
        util.cnfmDialogForText('提示', '蓝牙连接失败，是否使用打开蓝牙激活充电', function () {
          // 蓝牙激活充电
  ...
  bleReportRsp: function () {
    ...
    //2. 请求后台处理充电.
    util.wxRequsetForPost("wxapp", "reportForRecharge", {
      custNo: custNo,
      chargerId: chargerId,
      tradeId: tradeId,
      bleCode: bleCode,
  ...
  bleStateListener: function (state) {
    ...
    switch (state) {
      ...
      case constants.STATE_CLOSE_BLE: //蓝牙未打开 关闭状态
        //util.alertDialog('提示', '您的蓝牙未打开，请打开手机蓝牙和GPS位置定位才能使用充电器。', '确定', null)
        break;
      case constants.STATE_NOTBLE_WCHAT_VERSION: //微信版本过低 不支持ble
        util.alertDialog('提示', constants.NOT_PERMISSION, '确定', null)
        break;
  ...
  _autoJumpRdyToRchg: function () {
    ...
    let hTimeToRchg = setTimeout(function () {
      ...
      if (that.data.needBluetooth) {
        ...
        util.alertDialog('提示', '激活充电器失败，请打开手机蓝牙和GPS位置定位，靠近充电器重试', '确定', function () {
          that.bleConnectBluetooth();
        })
      } else {
  ...
  fnOpenBluetooth: function () {
    //开启蓝牙
    util.alertDialog('提示', '请打开手机蓝牙和GPS位置定位，靠近充电器重试', '确定', null);
  },
  /**
   * 读对充电时间读秒
   */
   ...
   * 广告图片点击
   */
  onAdImageClick: function () {
    // TODO 这里增加点击你的广告跳转功能，不需要可以删除
  }
})