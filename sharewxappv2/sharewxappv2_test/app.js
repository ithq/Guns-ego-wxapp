const utl = require('/utils/util.js')
const cmmn = require('/utils/cmmn.js')
App({
  //全局参数
  glbParam: {
    systemName: "共享充电",//系统名称
  ...
  isLogined: function (mustAlt) {
    if (this.glbParam.ssn_3rd == null || this.glbParam.ssn_3rd == undefined ||
      ...
      if (mustAlt) {
        try {
          utl.alertDialog("提示", "正在登录中，请稍后!", "确定", null);
        } catch (res) {
        }
      }
      return false;
  ...
  */
    ...
    utl.wxRequset("wxapp", "scanDevice", {
      ...
      if (rs.data.responseInfo) {
        ...
        if (pageIndex === 'MyRechargerPage') {
          ...
          that.endOperate(obj);
          utl.alertDialog('提示', '处理二维码信息异常，请确定扫码正确!', '确定', function () {
            if (confirmFun) {
              confirmFun();
            }
          })
        ...
        if (rs.data.result == 'error') {
          utl.alertDialog('提示', rs.data.message, '确定', function () {
            if (confirmFun) {
              confirmFun();
            }
          })
          ...
          utl.alertDialog('提示', '系统异常，请确认扫码正确，重新尝试!', '确定', function () {
            if (confirmFun) {
              confirmFun();
            }
          })
      ...
      function (msg) {
        that.endOperate(obj);
        utl.alertDialog('提示', '扫码异常，请确认扫码正确，重新尝试!', '确定', function () {
          if (confirmFun) {
            confirmFun();
          }
        })
  ...
  chargerGHAndConfirmFun: function (obj, deviceNo, chargerId, ConfirmFun) {
    ...
    utl.wxRequset("wxapp", "deviceGH.htm", {
      ...
      if (res.data.result == 'error') {
        ...
        utl.alertDialog('提示', res.data.message, '确定', function () {
          if (ConfirmFun) {
            ConfirmFun();
          }
        })
    ...
    wx.scanCode({
      success: (res) => {
        ...
        if (scanResult.indexOf('/dt/sbg.htm') > -1 || scanResult.indexOf("ID=") < 0) {
          utl.alertDialog('提示', '您扫描的二维码不正确, 请扫描充电宝上的二维码!', '确定', null)
        } else {
  ...
  backCharger: function (obj, deviceId) {
    ...
    utl.wxRequsetForPost("wxapp", "toBk.htm", {
      ...
      if (success.data.responseInfo == null) {
        utl.alertDialog('提示', success.data.message, '确定', that.moveToIndex)
        return;
      } else {
        //返回数据放缓存
      ...
      utl.alertDialog('提示', '网络超时，请重新尝试.', '确定', that.moveToIndex)
      that.endOperate(obj);
    }, function (complete) { })
  },
  ...
  getChargerIdByScanResult: function (scanResult) {
    if (scanResult == '') {
      //错误提示页面TODO
      utl.alertDialog('提示', '请扫描正确的二维码!', '确定', null)
      return;
    }
    scanResult = scanResult.toUpperCase();
    var begin = scanResult.indexOf('ID=');
