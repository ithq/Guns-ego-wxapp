...
Page({
  ...
  onLoad() {
    ...
    // 显示有效退款时间 
    var refundTime = 0;
    var faultReportText = "如果设备无法使用,请点击此按钮上报故障";
    if (responseInfo && responseInfo.config){
      refundTime = responseInfo.config.refundTime;
    }
    if (refundTime > 0){
      faultReportText += ",自动退款(" + refundTime + "分钟内有效)";
    }
    that.setData({
      chargerId: chargerId,
      deviceNo: deviceNo,
      ...
      faultReportText: faultReportText,
    ...
    utls.wxRequsetForPost("zfb", "replacePassWd", {
      ...
      if (rs.data.responseInfo) {
        ...
        if (pageIndex === 'MyRechargerPage') {
          ...
          try {
            my.setStorageSync('responseInfo', responseInfo);
          } catch (e) {
            try {
              my.setStorageSync('responseInfo', responseInfo);
            } catch (e) {
        ...
        } else if (pageIndex === 'ReadyToBleRecharge') {
          //生成跳转到蓝牙密码界面的参数对应...
          var responseInfo = rs.data.responseInfo;
          responseInfo.map = {};
          responseInfo.map.chargerId = that.data.chargerId;
          ...
          responseInfo.map.tradeId = that.data.tradeId;
          responseInfo.map.password = responseInfo.pwd;
          responseInfo.map.custNo = responseInfo.custNo;
          
          responseInfo.map.needBluetooth = responseInfo.needBluetooth;
          responseInfo.map.bleCode = responseInfo.bleCode;
          responseInfo.map.bleName = responseInfo.bleName;
          try {
            my.setStorageSync('responseInfo', responseInfo);
          } catch (e) {
            try {
          ...
          //跳转到输入密码界面...
          my.redirectTo({
            url: '/pages/rdyToBleRchg/rdyToBleRchg'
          })
        } else {
          app.endOperate(that);
  ...
  scanDeviceForTrade: function (obj, scanResult, confirmFun) {
    ...
    var keyCode = cmmn.getKeyCode([scanResult, tradeId, chargerId, deviceNo], app.glbParam.authCode)
    utls.wxRequset("zfb", "scanDeviceForTrade", {
      ...
      if (rs.data.responseInfo) {
        ...
        if (pageIndex === 'MyRechargerPage') {
          ...
          responseInfo.map.needBluetooth = responseInfo.needBluetooth ? true : false;
          responseInfo.map.bleCode = responseInfo.bleCode;
          responseInfo.map.bleName = responseInfo.bleName;
          try {
            my.setStorageSync({
