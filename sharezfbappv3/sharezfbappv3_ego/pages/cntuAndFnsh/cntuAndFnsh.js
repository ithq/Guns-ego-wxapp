...
Page({
  ...
  loadUsedInfo(){
    ...
    if (responseInfo && (responseInfo.pwdType == 1003 || responseInfo.pwdType == 1007)){
      // 预付款类型，直接显示结束订单
      closeOrderFlag = true;
  ...
  continueUsing(){
    ...
    responseInfo.map.needBluetooth = that.data.needBluetooth;
    responseInfo.map.bleCode = that.data.bleCode;
    responseInfo.map.bleName = that.data.bleName;
  ...
  overUsing(){
    ...
    //1.1 参数加密
    var keyCode = cmmn.getKeyCode([chargerId, custNo, tradeId], app.glbParam.authCode)
