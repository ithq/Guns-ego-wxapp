...
Page({
  ...
  loadView(){
    ...
    if (responseInfo != null && responseInfo != undefined
      ...
      // 蓝牙数据
      needBluetooth = responseInfo.map.needBluetooth;
      bleCode = responseInfo.map.bleCode;
      bleName = responseInfo.map.bleName;
    }
    // 加载广告图 
    var advertImage = '/assets/img/rdyToBleRchgBg.jpg';
  ...
  onPlayVoice: function () {
    ...
    // 这里要用优酷的src="/img/voice/reblecharger.mp3"
    that.data.audioContext.src = "XNDc3MjE5ODg4MA==";
  ...
  openBluetooth() {
    ...
    let res = my.getStorageSync({ key: 'currentBle' });
    if (res && res.data){
      // 初始化蓝牙
      bletools.setCurrentBle(this, res.data);
   ...
   * 蓝牙扫描，匹配，连接
   */
  bleConnectBluetooth: function (count) {
    let that = this;
  ...
  bleReportRsp: function () {
    let that = this;
    //1.1 参数加密
    var code = app.glbParam.authCode;
    ...
    //2. 请求后台处理充电.
    utls.wxRequsetForPost("zfb", "reportForRecharge", {
      custNo: custNo,
      chargerId: chargerId,
      tradeId: tradeId,
      bleCode: bleCode,
  ...
  _autoJumpRdyToRchg: function () {
    ...
    if (that.data.needBluetooth) {
      ...
      if(that.data.scanBleCount >= 2){
        // 重试2次搜索，此时已然没有充电成功，跳转到密码界面进行充电
        that.fnGetPassWord();
      } else {
        // 搜索2次
        that.bleConnectBluetooth();
   ...
   */
  clickAdvertInfo(e){
    app.clickAdvertInfo(5, this.data.chargerId);
  },
});