...
// var rqstBaseUrl = "https://it.frp.szego.net/"
var rqstBaseUrl = "https://ewx.szego168.com/rest/"
...
function getSessionId() {
  var sessionId = null;
  try {
    let res = my.getStorageSync({ key: 'sessionId' });
    if (res.data) {
      sessionId = res.data;
    }
  } catch (e) {
 ...
 * completeFun: 完成事件
 */
function alert(title, content, buttonText, btnFun, completeFun){
  my.alert({
    title: title,
    content: content,
...
function wxRequset(ctrlMapping, rqstMapping, params, successFun, failFun, completeFun) {
  var sss_3id = "";
  try {
    sss_3id = my.getStorageSync('ssn_3rd');
  } catch (e) {
    try {
      sss_3id = my.getStorageSync('ssn_3rd');
    } catch (e) {
...
function wxRequsetForPost(ctrlMapping, methodMapping, params, scsFun, failFun, completeFun) {
  ...
  try {
    sessId = my.getStorageSync('sessId');
  } catch (e) {
    try {
      sessId = my.getStorageSync('sessId');
    } catch (e) {}
...
function showLoading(msg) {
  try {
    if (my.canIUse("showLoading")) {
      my.showLoading({content: msg});
    }
  } catch (eee) { }
}
/**
