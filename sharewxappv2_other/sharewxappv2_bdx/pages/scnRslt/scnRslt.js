...
Page({
  ...
  data: {
    ...
    bleOpenBluetoothFlag: false, //是否已开启蓝牙
  },
    ...
    utls.wxRequsetForPost("wxapp", "getPasswordForRecharge",
      {
        ...
        that.innerAudioContext.pause();
        //4. 跳转到显示密码界面或者蓝牙充电界面
        if (success.data.responseInfo && success.data.responseInfo.pageIndex == "ReadyToBleRecharge"){
          wx.redirectTo({
  ...
  bleOpenBluetoothEvent: function(res){
    let that = this;
    if (res){
      if (!that.data.bleOpenBluetoothFlag) {
        that.setData({ bleOpenBluetoothFlag: true });
      }
    }else{
      that.setData({ bleOpenBluetoothFlag: false });
    }
  },
  /**
   * ble状态监听
  ...
  myWantToReCharge: function (e) {
    ...
    if (that.data.needBluetooth && that.data.bleOpenBluetoothFlag == false) {
      utls.altDialog('提示', constants.NOT_BLE, '确定', null);
      return;
    }
