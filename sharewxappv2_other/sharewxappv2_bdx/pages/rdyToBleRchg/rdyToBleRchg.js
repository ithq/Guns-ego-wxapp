...
Page({
  ...
  data: {
    ...
    txtForTime: "00:00:00", //使用时长
    showTitle: '请开启手机蓝牙，靠近充电器解锁充电',
  ...
  notifyListener: function (data) {
    ...
    //接收数据上报到后台
    console.info('6.上报后台');
    if (bleCodeResult.toUpperCase() == that.data.bleCode) {
      return;
    }
    that.setData({
      bleCodeResult: bleCodeResult
    });
    that.bleReportRsp();
  ...
  _autoJumpRdyToRchg: function () {
    ...
    let hTimeToRchg = setTimeout(function () {
      ...
      if (that.data.needBluetooth) {
        ...
        utls.altDialog('提示', '解锁充电器失败，请打开手机蓝牙后，靠近充电器重试', '确定', function () {
          that.bleConnectBluetooth();
        })
      } else {
  ...
  fnOpenBluetooth: function () {
    //开启蓝牙
    utls.altDialog('提示', '请打开手机蓝牙后，靠近充电器重试', '确定', null);
  },
  /**
   * 读对充电时间读秒
   */
  readSecondForRecharge: function() {
    ...
    var txtForTime = this.pad(hours, 2) + ":" + this.pad(minutes, 2) + ":" + this.pad(seconds, 2);
    this.setData({
      haveUsedSeconds: haveUsedSeconds,
      txtForTime: txtForTime
    })
  },
  /**
   * 右边截取
