 ...
 * successGetUserInfoEventCallback:成功授权拿到用户信息后回调
...
App({
  ...
  onLaunch: function (options) {
    ...
    console.log(options);
    // 5.初始化系统时间
    that.glbParam.timesForLaunch = (new Date()).getTime();
  },
  //显示页面 ..
  onShow: function (options) {
    console.log(options);
    if (options && options.scene === 1038) { // 场景值1038：从被打开的小程序返回
      const { appId, extraData } = options.referrerInfo;
      if(extraData && extraData.query_id){
        this.glbParam.queryId = extraData.query_id;
      }
    }
  },
  //隐...
  ...
  refusedGetUserInfoWithUser: function () {
    this.refusedForGetUserInfoByMsg("拒绝授权获取微信昵称等信息，部分功能您将无法使用,是否重新授权！");
  },
  /*如果用户不同意，授权获取头像等资源，调用此方法提示用户...*/
  ...
  */
  ...
  doLoginFoServerEvent: function (usrInfo) {
    ...
    utl.wxRequsetForPost("whitelist", "wxappLogin", {
      ...
      if (result == "success") {
        //1.分析返回参数成功
        var sssId=rs.data.responseInfo.sess || rs.data.responseInfo;
        var advertInfoMap = rs.data.responseInfo.advertInfoMap;
        var userInfo = rs.data.responseInfo.userInfo;
        var arySession = sssId.split("_");
        //2. 保存参数session。。
        that.glbParam.ssn_3rd = arySession[0];
        that.glbParam.advertInfoMap = advertInfoMap;
        that.glbParam.currentUserInfo = userInfo;
        try {
        ...
        if (that.successLoginEventForCallback) {
          that.successLoginEventForCallback(rs.data.responseInfo);
        }
      } else {
  ...
  lgnAfterEventCallMethod: function (mthd) {
    ...
    if (this.glbParam.ssn_3rd == null || this.glbParam.ssn_3rd == undefined || this.glbParam.ssn_3rd == "") {
      if (!this.glbParam.credentForGetUsrInfo) {
        this.refusedForGetUserInfoByMsg("您未同意授权获取您的昵称等信息，此功能不可用，是否重新允许?");
  ...
  */
  ...
  //获取广告信息 type:1首页顶部，2首页中部 3我要充电 4密码界面 5蓝牙界面
  ...
  clickAdvertInfo: function(type, businessId){
    ...
    if(advertInfo){
      if(advertInfo.navigateTo == "url"){
        // 广告点击
        var url = advertInfo.url;
        url = url.replace('{custNo}', that.glbParam.custNo);
   ...
   * 弹出插屏广告
   */
  showInterstitial(object, adUnitId){
    let that = this;
    ...
    // 在小程序刚加载时，延迟加载广告
    var timeout = 2000;
    if(((new Date()).getTime() - that.glbParam.timesForLaunch) > 2000){
      timeout = 500;
    }
    // 在适合的场景显示插屏广告
    setTimeout(function(){
      if (interstitialAd && object.data.isShow) {
