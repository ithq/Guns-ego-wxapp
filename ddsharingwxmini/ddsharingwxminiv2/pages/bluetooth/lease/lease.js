...
Page({
      ...
      success: function (res) {
        ...
        wx.onBluetoothDeviceFound(function (res) {          //监听搜索到设备
          ...
          if(res.devices[0].localName){//有些其他的设备没有localName,避免报错
            if (res.devices[0].localName.indexOf(servieID) != -1) {
              // wx.hideLoading();
    ...
    wx.getBLEDeviceCharacteristics({
      ...
      success: function (res) {
        ...
        for (var index = 0; index < characteristics_length; index++) {
          ...
          /* 判断是否是我们需要的FEE4 */
          if (characteristics_slice == 'FFF4' || characteristics_slice == 'fff4') {       //可读可写 
        ...
        wx.onBLECharacteristicValueChange((characteristic) => {
          ...
          //根据密码位数判断是否解析成功
          var sliceStr = hex.slice(4, 6);
          // sliceStr = parseInt(sliceStr);
          sliceStr = that.ex16hex(sliceStr);
          console.log(hex);
  ...
  write: function (buffer) {
    wx.writeBLECharacteristicValue({
      deviceId: connectID,
      serviceId: serviceID,
      characteristicId: notycharacteristicsId,//写入FFF4
      value: buffer,
      success: function (res) {
