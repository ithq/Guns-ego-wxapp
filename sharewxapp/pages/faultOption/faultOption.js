...
Page({
  ...
  onLoad: function (options) {
    ...
    // 只有万能版(id=14)展示更新密码
    if (null != responseInfo.shareChargerModel && undefined != responseInfo.shareChargerModel 
      && responseInfo.shareChargerModel.chargerTypeId == 14){
  ...
  replacePassWd: function(e){
    ...
    util.shareRequestPost("wxapp", "replacePassWd", {
      ...
      if (rs.data.responseInfo) {
        ...
        if (pageIndex === 'MyRechargerPage') {
          ...
          //跳转到输入密码界面...
          wx.navigateTo({
            url: '/pages/readyToRecharge/readyToRecharge'
          })
        } else {
          app.finishOperation(that);
          util.alterDialog('提示', '处理二维码信息异常，请确定扫码正确!', '确定', null);
        }
      }else{
        app.finishOperation(that);
  ...
  scanDeviceForTrade: function (obj, scanResult, confirmFun) {
    ...
    var keycode = common.getKeyCode([scanResult, tradeId, chargerId, deviceNo], app.globalData.code)
    util.shareRequest("wxapp", "scanDeviceForTrade", {
