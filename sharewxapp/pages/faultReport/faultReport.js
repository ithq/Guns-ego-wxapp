...
Page({
  ...
  onLoad: function () {
    ...
    wx.getSystemInfo({
      success: function (res) {
        var info = '---\r\n**用户信息**\r\n';
        info += '用户名：' + app.globalData.userInfo.nickName;
        info += '\r\n手机型号：' + res.model;
  ...
  uploadPic: function () {
    ...
    if (that.data.hasImage1 && that.data.hasImage2 && that.data.hasImage3) {
      util.alterDialog('提示', "图片只能上载3张!", '确定', null);
    }
    wx.chooseImage({
      count: 3, // 默认9
  ...
  uploadAliyun: function (tempFilePath, filename) {
    wx.uploadFile({
      ...
      fail: function (errMsg) {
        ...
        util.alterDialog('提示', "上传失败", '确定', null);
      },
    })
  },
  ...
  getSignature: function (tempFilePath, filename) {
    var keycode = common.getKeyCode([filename], app.globalData.code);
    util.shareRequest("fileUpload", "getParam", {
  ...
  submitForm: function (e) {
    ...
    //先进行表单非空验证
    if (tel == null) {
      util.alterDialog('提示', "请输入手机号", '确定', null);
      return;
    }
    if (!util.validateValue("Mobile", tel)) {
      util.alterDialog('提示', "请输入正确的手机号", '确定', null);
      return;
    }
    if (questNote == "") {
      util.alterDialog('提示', "请输入您遇到的问题", '确定', null);
      return;
    }
    wx.showModal({
      title: '提示',
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          util.shareRequestPost("wxapp", "faultReport", {
            session3rd: app.globalData.session_3rd,
            keyCode: keycode,
            chargerId: chargerId,
            deviceNo, deviceNo,
            ...
            if (rs.data.result == "success") {
              util.alterDialog('提示', '故障反馈成功', '确定', function (res){
                // //跳转首页界面...
                wx.navigateTo({
              ...
              if (rs.data.result == 'error') {
                ...
                util.alterDialog('提示', '系统异常，请重新尝试!', '确定', null);
              }
            }
